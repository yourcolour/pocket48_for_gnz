...
class ModianEntity:
    def __init__(self, link, title, pro_id, order_flag, need_display_rank=False, current=0.0, target=0.0, support_num=0):
        self.link = link
        self.title = title
        self.pro_id = pro_id
        ...
        for order in orders:
            ...
            for jiebang in jiebang_activities:
                ...
                if backer_money >= jiebang.min_stick_amount:
                    ...
                    if len(test_msg) > 0:
                        msg += test_msg
                        QQHandler.send_to_groups(self.modian_notify_groups, test_msg)
